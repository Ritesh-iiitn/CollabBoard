...
export default function Whiteboard({ roomId, userEmail }: WhiteboardProps) {
  ...
  const joinAudioRoom = async () => {
    try {
      // 1. Request microphone permission beforehand to prevent connection timeouts during popup delay
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        // Close the stream immediately; LiveKit will open its own
        stream.getTracks().forEach(track => track.stop());
      } catch (permissionErr) {
        console.warn("Microphone permission denied or device not found:", permissionErr);
        toast.error("Please allow microphone access to publish your audio.");
        // We will continue anyway, in case they just want to join as a listener
      }
      ...
      // Enable microphone by default (wrapped in a try-catch to keep the connection open even if publishing fails)
      try {
        await room.localParticipant.setMicrophoneEnabled(true);
        setIsMuted(false);
        toast.success("🎙️ Joined call");
      } catch (pubErr) {
        console.error("Failed to publish local audio track:", pubErr);
        setIsMuted(true);
        toast.success("🎙️ Joined call (muted)");
      }
    } catch (err) {
      console.error(err);
...
const VideoParticipant = ({ participant, isLocal = false }: { 
  ...
  useEffect(() => {
    ...
    const handleConnect = () => {
      console.log("Client socket connected! Emitting userJoined for room:", roomId);
